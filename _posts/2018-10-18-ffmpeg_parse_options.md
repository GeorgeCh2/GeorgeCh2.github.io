                      ...
                      const OptionGroupDef *groups, int nb_groups)
{
    ...
    while (optindex < argc) {
        ...
        if (opt[0] != '-' || !opt[1] || dashdash+1 == optindex) {
            /** 
             * 将输出的路径、 octx->cur_group数组的值以及专属参数，存储到      			     * octx->groups[0]结构体中,并清空 octx->cur_group 数组的值
             */
...
do {                                                                           \
    ...
    if (!arg) {                                                                \
        ...
        if ((ret = match_group_separator(groups, nb_groups, opt)) >= 0) {
            ...
            GET_ARG(arg);
            /** 
             * 将输入的路径、octx->cur_group数组的值以及专属参数，存储到      			     * octx->groups[0]结构体中,并清空 octx->cur_group 数组的值
             */
        ...
        if (argv[optindex]) {
            /**
             * 在 avcodec_options、avformat_options、avresample_options、   			 * swscale_options、swresample 中的 options 参数中不断查找，查找专属			  * 并存入 AVDictionary
             */
